...
int AnimatedDrawable::id = 0;
void AnimatedDrawable::drawObject()
{
  if (textureFilePathArray.size() != 0)
  {
    glEnable(GL_TEXTURE_2D);
    
    glBindTexture(GL_TEXTURE_2D, textureIdArray[currentTexture]);
  ...
  {
    currentTexture ++;
    if( currentTexture >= textureIdArray.size() )
      currentTexture = 0;
    frameCount = 0;
  }
